        ...
        describe('functional tests', () => {
            ...
            describe('Grant authorization', () => {
                it('allows an external party to ProtocolsQuery only if they have a valid grant', () => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const { message: publicProtocolMessage } = yield TestDataGenerator.generateProtocolsConfigure({
                        author: alice,
                        published: true,
                    });
                    const { status: publicProtocolStatus } = yield dwn.processMessage(alice.did, publicProtocolMessage);
                    expect(publicProtocolStatus.code).to.equal(202);
                    const { message: privateProtocolMessage } = yield TestDataGenerator.generateProtocolsConfigure({
                        author: alice,
                        published: false,
                    });
                    const { status: privateProtocolStatus } = yield dwn.processMessage(alice.did, privateProtocolMessage);
                    ...
                    expect((_a = protocolsQueryReply.entries) === null || _a === void 0 ? void 0 : _a.length).to.equal(2);
                    // 3. Verify that Mallory cannot to use Bob's permission grant to gain access to Alice's DWN
                ...
                it('should allow to scope a ProtocolsQuery to a specific protocol', () => __awaiter(this, void 0, void 0, function* () {
                    var _b, _c, _d, _e;
                    const alice = yield TestDataGenerator.generateDidKeyPersona();
                    const bob = yield TestDataGenerator.generateDidKeyPersona();
                    // create 2 unpublished protocols, and one published protocol
                    const { message: allowedProtocolMessage } = yield TestDataGenerator.generateProtocolsConfigure({
                        author: alice,
                        published: false,
                    });
                    const allowedProtocol = allowedProtocolMessage.descriptor.definition.protocol;
                    const { status: allowedStatus } = yield dwn.processMessage(alice.did, allowedProtocolMessage);
                    expect(allowedStatus.code).to.equal(202);
                    const { message: notAllowedProtocolMessage } = yield TestDataGenerator.generateProtocolsConfigure({
                        author: alice,
                        published: false,
                    });
                    const notAllowedProtocol = notAllowedProtocolMessage.descriptor.definition.protocol;
                    const { status: notAllowedStatus } = yield dwn.processMessage(alice.did, notAllowedProtocolMessage);
                    expect(notAllowedStatus.code).to.equal(202);
                    const { message: publishedProtocolMessage } = yield TestDataGenerator.generateProtocolsConfigure({
                        author: alice,
                        published: true,
                    });
                    const publishedProtocol = publishedProtocolMessage.descriptor.definition.protocol;
                    const { status: publishedStatus } = yield dwn.processMessage(alice.did, publishedProtocolMessage);
                    ...
                    const protocolsQueryAllowed = yield TestDataGenerator.generateProtocolsQuery({
                        author: bob,
                        filter: {
                    ...
                    const protocolQueryAllowedReply = yield dwn.processMessage(alice.did, protocolsQueryAllowed.message);
                    expect(protocolQueryAllowedReply.status.code).to.equal(200);
                    expect((_b = protocolQueryAllowedReply.entries) === null || _b === void 0 ? void 0 : _b.length).to.equal(1);
                    ...
                    const protocolQueryNotAllowed = yield TestDataGenerator.generateProtocolsQuery({
                        author: bob,
                        filter: {
                    ...
                    const protocolQueryNotAllowedReply = yield dwn.processMessage(alice.did, protocolQueryNotAllowed.message);
                    expect(protocolQueryNotAllowedReply.status.code).to.equal(200);
                    expect((_c = protocolQueryNotAllowedReply.entries) === null || _c === void 0 ? void 0 : _c.length).to.equal(0);
                    // Bob tries to ProtocolsQuery to Alice's DWN for a published protocol with the same grant
                    const protocolQueryPublished = yield TestDataGenerator.generateProtocolsQuery({
                        author: bob,
                        filter: {
                    ...
                    const protocolQueryPublishedReply = yield dwn.processMessage(alice.did, protocolQueryPublished.message);
                    expect(protocolQueryPublishedReply.status.code).to.equal(200);
                    expect((_d = protocolQueryPublishedReply.entries) === null || _d === void 0 ? void 0 : _d.length).to.equal(1);
                    ...
                    const protocolQueryNoFilters = yield ProtocolsQuery.create({
                        signer: Jws.createSigner(bob),
                        permissionGrantId: permissionGrant.recordsWrite.message.recordId,
                    });
                    const protocolQueryNoFiltersReply = yield dwn.processMessage(alice.did, protocolQueryNoFilters.message);
                    expect(protocolQueryNoFiltersReply.status.code).to.equal(200);
                    expect((_e = protocolQueryNoFiltersReply.entries) === null || _e === void 0 ? void 0 : _e.length).to.equal(1);
