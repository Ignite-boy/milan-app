            ...
            if (actionRule.role !== undefined) {
                ...
                if (!roles.includes(actionRule.role)) {
                    ...
                    if (readActions.find(action => actionRule.can.includes(action)) && !readActions.every(action => actionRule.can.includes(action))) {
                        throw new DwnError(DwnErrorCode.ProtocolsConfigureRoleReadActionMissing, `Role in action ${JSON.stringify(actionRule)} for rule set ${ruleSetProtocolPath} must contain all read actions (${readActions.join(', ')}).`);
                    }
                }
            }
